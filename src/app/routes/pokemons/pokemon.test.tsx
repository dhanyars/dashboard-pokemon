...
let filters = { search: "", sort: "name", itemsPerPage: 20, currentPage: 1 };
const useLoadPage = jest.fn();
...
const rendered = (pokemon: any, filters: any, useLoadPage: any) => {
  return render(
...
describe("Pokemon component", () => {
  it("shows title and description", () => {
    const { getByText } = rendered(pokemon, filters, useLoadPage);
    expect(getByText("Pokemon Dashboard")).toBeTruthy();
    expect(getByText("Search pokemon")).toBeTruthy();
  });

  it("shows filtering with correct value", () => {
    filters = {
      search: "bulbasaur",
      sort: "name",
      itemsPerPage: 20,
    ...
    expect(getByTestId("pokeon-name-bulbasaur").textContent).toBe("bulbasaur");
    expect(queryByTestId("pokeon-name-bulbasaur")).toBeTruthy();
  });

  it("shows filtering with wrong value", () => {
    filters = {
      search: "abc",
      sort: "name",
      itemsPerPage: 20,
    ...
    const { queryByTestId } = rendered(pokemon, filters, useLoadPage);
    //expect(getByTestId("pokeon-name-bulbasaur").textContent).toBe("bulbasaur");
    expect(queryByTestId("pokeon-name-bulbasaur")).toBeFalsy();
  });

  it("shows no pokemon found if search value is not present", () => {
    filters = {
      search: "abc",
      sort: "name",
      itemsPerPage: 20,
